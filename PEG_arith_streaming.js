p_arith_streaming_v6_Expr.names=['','Expr','Add','Mult','Num','S','_']
function p_arith_streaming_v6_Expr(out){var eof=false,s='',l=0,S=24576,T,M,F,R,tbl=[],x,pos=0,offset=0,buf=[],bufs=[],states=[],posns=[],c;
...
if(typeof out=='string'){s=out;out=[];x=p_arith_streaming_v6_Expr(function(m,x,y){if(m=='fail')out=[false,x,y];if(m=='tree segment')out=out.concat(x)});x('chunk',s);x('eof');return out[0]===false?out:[true,{names:p_arith_streaming_v6_Expr.names,tree:out,input:s}]}
