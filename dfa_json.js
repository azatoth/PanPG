...
{_:{S:4,name:'_',all_csets:[[0]],known_regular:[true]
   ,expr:{id:'_+0',type:2,S:4,T:28672,M:10,F:11,toplevel:true,flag_n:0
         ,S_flags:16384,S_flags_:'100000000000000'
         ...
         ,subexprs:[{id:'_+1',type:5,S:7,T:6831,M:32768,F:11
                    ...
                    ,S_flags:28672,S_flags_:'111000000000000'}
                   ,{id:'_+2',type:6,S:8,T:36864,M:10,F:11
                    ,flags:undefined,flag_n:0,S_flags:32768
                    ,S_flags_:'1000000000000000'
                    ,subexprs:[{id:'_+3',type:0,S:9,T:14,M:11,F:10
                               ,flags:undefined,cset:[0],flag_n:0
                               ,subexprs:[],S_flags:36864
...
,JSON:{S:1,re:[2,[[5,'Digit']]],name:'JSON',direct_deps:['Digit']
      ...
      ,expr:{id:'JSON+0',type:2,S:1,T:20480,M:10,F:11,toplevel:true
            ...
            ,S_flags:6831,S_flags_:'1101010101111'
...
,Digit:{S:2,re:[2,[[0,[48,58]]]],name:'Digit',direct_deps:[]
       ...
       ,expr:{id:'Digit+0',type:2,S:2,T:24576,M:10,F:11,toplevel:true
             ...
             ,dfa:undefined,S_flags:10927,S_flags_:'10101010101111'
             ,flags:{cache:true,t_bufferout:true,pushpos:true
                    ,t_emitstate:true,m_emitstate:false,m_emitclose:true
                    ,m_emitanon:false,m_emitlength:true,m_resetpos:false
                    ,m_emitbuf:true,m_tossbuf:false,f_tossbuf:true}
...
function parseJSON(out){var eof=false,s='',l=0,S=16384,T,M,F,D,R,tbl=[],x,pos=0,offset=0,buf=[],bufs=[],states=[],posns=[],c,equiv,S_map=['','JSON+0','Digit+0','','_+0','JSON+1','Digit+1','_+1','_+2','_+3'];
function dbg(msg){out(msg,'S:'+(S_map[S>>>12]||'S>>>12)+' pos:'+pos+' '+s.charAt(pos)'+' R:'+R+' stack:'+states.map(function(s){return s>>>12})+' posns:'+posns+' bufs:'+bufs.map(function(b){return '['+b+']'})+' buf:'+buf)}
...
D=[,,,,,,[[[[1]]]]]
function revive(dfa){var i,l,state,j,l2,all=[],t,ts;for(i=0,l=dfa.length;i<l;i++){state=dfa[i];ts=[];for(j=0,l2=state.length;j<l2;j++){t=state[j];if(state[1]==l) ts.push([t[0],true]);else ts.push([t[0],t[1]==undefined?j+1:t[1]])}all.push(ts)}return dfa(all)
 function dfa(ss){var i,l_ss,st,l_s,t,l_t,a,d=[],j,k,l;for(i=0,l_ss=ss.length;i<l_ss;i++){st=ss[i];a=[];for(j=0,l_s=st.length;j<l_s;j++){t=st[j];for(k=0,l_t=t[0].length;k<l_t;k++){a[t[0][k]]=t[1]==true?l_ss:t[1]}}for(j=0,l=a.length;j<l;j++)if(a[j]==undefined)a[j]=l_ss+1;d[i]=a}
  return function _dfa(){var st=0,eq,i=undefined;while(st<l_ss){eq=undefined[undefined.charCodeAt(i++)];st=d[st][eq]}if(st==l_ss){undefined=i;return true}return false}}}
...
if(S&1/*cache*/&&(x=tbl[pos-offset][S])!=undefined){if(x){R=true;pos=x[0];buf=x[1]}else{R=false}dbg("cache hit")break t_block}
